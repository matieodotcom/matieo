...
export default function AdminMemorialsPage() {
  ...
  const { data, isLoading, error } = useAdminMemorials({ page })
  const { mutate: setStatus } = useAdminSetMemorialStatus()
  const { mutate: deleteMemorial } = useAdminDeleteMemorial()
  ...
  const handleDelete = (id: string) => {
    setMutationError(null)
    deleteMemorial(id, { onError: () => setMutationError('Failed to delete memorial.') })
  }

  return (
    <div>
      <h1 className="text-2xl font-bold text-neutral-900 dark:text-neutral-100 mb-6">Memorials</h1>

      {mutationError && <ErrorMessage message={mutationError} />}
      {error && <ErrorMessage message="Failed to load memorials." />}
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="bg-white dark:bg-neutral-900 divide-y divide-neutral-100 dark:divide-neutral-800">
            {isLoading ? (
              ...
              <tr>
                <td colSpan={5} className="px-4 py-12 text-center text-neutral-400">No memorials found.</td>
              </tr>
            ) : (
              items.map((m) => (
                <tr key={m.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-colors">
                  <td className="px-4 py-3 font-medium text-neutral-900 dark:text-neutral-100">{m.full_name}</td>
                  <td className="px-4 py-3">
                    <Badge variant={m.status === 'published' ? 'success' : 'warning'}>
                      {m.status}
                    </Badge>
                  </td>
                  <td className="px-4 py-3 text-neutral-500 dark:text-neutral-400 font-mono text-xs">{m.slug ?? '—'}</td>
                  <td className="px-4 py-3 text-neutral-600 dark:text-neutral-400">{formatDate(m.created_at)}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <button
                            aria-label={`Delete memorial for ${m.full_name}`}
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogDescription className="mt-2 text-sm text-neutral-500 dark:text-neutral-400">
                            Are you sure you want to permanently delete <strong>{m.full_name}</strong>'s memorial? This action cannot be undone.
                          </AlertDialogDescription>
      ...
      {pages > 1 && (
        <div className="mt-4 flex items-center justify-between text-sm text-neutral-500">
          <span>{total} memorials total</span>
