...
export default function AdminCondolencesPage() {
  ...
  const { data, isLoading, error } = useAdminCondolences({ page })
  const { mutate: deleteCondolence } = useAdminDeleteCondolence()
  ...
  const handleDelete = (id: string) => {
    ...
    deleteCondolence(id, { onError: () => setMutationError('Failed to delete condolence.') })
  }

  return (
    <div>
      <h1 className="text-2xl font-bold text-neutral-900 dark:text-neutral-100 mb-6">Condolences</h1>

      {mutationError && <ErrorMessage message={mutationError} />}
      {error && <ErrorMessage message="Failed to load condolences." />}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-neutral-100 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900">
              ...
              <th className="px-4 py-3 text-left font-semibold text-neutral-600 dark:text-neutral-400">Message</th>
              <th className="px-4 py-3 text-left font-semibold text-neutral-600 dark:text-neutral-400">Obituary ID</th>
          ...
          <tbody className="bg-white dark:bg-neutral-900 divide-y divide-neutral-100 dark:divide-neutral-800">
            {isLoading ? (
              ...
              <tr>
                <td colSpan={5} className="px-4 py-12 text-center text-neutral-400">No condolences found.</td>
              </tr>
            ) : (
              items.map((c) => (
                <tr key={c.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-colors">
                  <td className="px-4 py-3 font-medium text-neutral-900 dark:text-neutral-100">{c.author_name ?? 'Anonymous'}</td>
                  ...
                  <td className="px-4 py-3 text-neutral-500 dark:text-neutral-400 font-mono text-xs">{c.obituary_id}</td>
                  <td className="px-4 py-3 text-neutral-600 dark:text-neutral-400">{formatDate(c.created_at)}</td>
                  <td className="px-4 py-3">
                    <AlertDialog>
                      <AlertDialogTrigger asChild>
                        <button
                          aria-label="Delete condolence"
                      ...
                      <AlertDialogContent>
                        <AlertDialogTitle>Delete Condolence</AlertDialogTitle>
                        <AlertDialogDescription className="mt-2 text-sm text-neutral-500 dark:text-neutral-400">
                          Are you sure you want to permanently delete this condolence message? This action cannot be undone.
                        </AlertDialogDescription>
      ...
      {pages > 1 && (
        <div className="mt-4 flex items-center justify-between text-sm text-neutral-500">
          <span>{total} condolences total</span>
