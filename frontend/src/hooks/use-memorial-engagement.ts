 ...
 */
export function useTrackMemorialView(id: string) {
  useEffect(() => {
    if (!id) return
    apiFetch<ViewResponse>(`/api/memorials/${id}/view`, { method: 'POST' }).catch(() => {
 ...
 * Optimistically updates the ['memorial', slug] query cache.
 */
export function useLikeMemorial(id: string, slug: string) {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: () =>
      apiFetch<LikeResponse>(`/api/memorials/${id}/like`, { method: 'POST' }),
    onSuccess: (res) => {
