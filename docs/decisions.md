...
## 2026-03-11 — In-app notification system: Radix Dialog drawer + Supabase Realtime

**Decision:** Notifications delivered via a right-side slide-in drawer (Radix `Dialog`, not `Popover`) with a bell icon + unread count badge in `Navbar` and `DashboardLayout`. Data fetched via TanStack Query (`GET /api/notifications`) with a Supabase Realtime `postgres_changes` subscription for live updates and a 60 s polling fallback. Created server-side via `createNotification()` in `notificationsClient.ts`, called fire-and-forget (IIFE with `.catch()`) from tributes, condolences, memorials, and obituaries controllers — same pattern as email. Self-notify guard applied: page owners do not receive notifications for their own actions.
**Why:** Radix Dialog gives a full-height accessible drawer (focus trap, Escape key, `aria-modal`) using an already-installed package — no new Radix dependency. Fire-and-forget mirrors the existing email pattern and keeps controller response time unaffected. Supabase Realtime provides instant badge updates without a separate WebSocket server. Polling fallback covers edge cases where the Realtime connection drops.
**Alternatives considered:** Radix Popover (dropdown) — rejected in favour of Dialog drawer for better mobile UX and more display space for notification content. Separate WebSocket server — overkill given Supabase Realtime is already available. Server-Sent Events — more complex infrastructure with no added benefit.
**Consequences:** `notifications` table requires `ALTER PUBLICATION supabase_realtime ADD TABLE public.notifications` after migration. `obituaries.created_by` is nullable (`ON DELETE SET NULL`) — all controllers must guard `if (!obituary.created_by)` before calling `createNotification`. Bell renders null for unauthenticated users; `useNotifications` query is disabled when `userId` is null.
